...
func checkForModification(folderPath string, excludedPaths ...string) error {
	fmt.Println("Started watching:", folderPath)

	lastSnap, err := buildSnapshot(folderPath, excludedPaths)
	if err != nil {
	...
	for {
		newSnap := waitForChange(lastSnap, lastMod, folderPath, excludedPaths)
...
func waitForChange(lastSnap []FileSnapshot, lastMod time.Time, root string, excludedPaths []string) []FileSnapshot {
	...
	go func() {
		...
		for {
			<-ticker.C
			newSnap, err := buildSnapshot(root, excludedPaths)
			if err != nil {
			...
			if !newMod.Equal(lastMod) || !snapshotsEqual(lastSnap, newSnap) {
				changed <- newSnap
				return
			}
		}
...
func buildSnapshot(root string, excludedPaths []string) ([]FileSnapshot, error) {
